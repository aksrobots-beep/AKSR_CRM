...
router.get('/', async (req, res) => {
  try {
    ...
    send500(res, 'Failed to get suppliers', error);
  }
});
...
router.get('/:id', async (req, res) => {
  try {
    ...
    send500(res, 'Failed to get supplier', error);
  }
});
...
router.post('/', async (req, res) => {
  try {
    ...
    send500(res, 'Failed to create supplier', error);
  }
});
...
router.put('/:id', async (req, res) => {
  try {
    ...
    send500(res, 'Failed to update supplier', error);
  }
});
...
router.patch('/:id/active', async (req, res) => {
  try {
    ...
    send500(res, 'Failed to toggle supplier status', error);
  }
});
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    send500(res, 'Failed to delete supplier', error);
  }
});
