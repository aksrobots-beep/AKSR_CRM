...
router.get('/', async (req, res) => {
  try {
    ...
    send500(res, 'Failed to get employees', error);
  }
});

// Create new employee record (called after user creation)
router.post('/', requireRole('ceo', 'admin', 'hr_manager'), async (req, res) => {
  try {
    const { user_id, position, join_date, salary, annual_leave_balance, sick_leave_balance } = req.body;
    ...
    let validatedJoinDate = join_date;
    if (join_date) {
      const r = validateDate(join_date, { required: false, fieldName: 'Join date' });
      if (!r.valid) return res.status(400).json({ error: r.error });
      validatedJoinDate = r.value;
    }
    ...
    const empRecord = {
      id: uuidv4(),
      user_id,
      position: position || user.role.replace('_', ' '),
      join_date: validatedJoinDate || now,
      salary: salary || 0,
      annual_leave_balance: annual_leave_balance ?? 14,
      sick_leave_balance: sick_leave_balance ?? 14,
      created_at: now,
    ...
    res.status(201).json({
      ...
      join_date: empRecord.join_date,
      salary: empRecord.salary,
      annual_leave_balance: empRecord.annual_leave_balance,
      sick_leave_balance: empRecord.sick_leave_balance,
      is_active: user.is_active,
      updated_at: empRecord.updated_at,
    });
  } catch (error) {
    console.error('Create employee error:', error);
    send500(res, 'Failed to create employee record', error);
  }
});
...
router.get('/:id', async (req, res) => {
  try {
    ...
    send500(res, 'Failed to get employee', error);
  }
});
...
router.put('/:id', requireRole('ceo', 'admin', 'hr_manager'), async (req, res) => {
  try {
    ...
    const updates = { position, join_date: validatedJoinDate, salary, annual_leave_balance, sick_leave_balance, updated_at: now, updated_by: req.user.id };
    ...
    send500(res, 'Failed to update employee', error);
  }
});
...
router.patch('/:id/active', requireRole('ceo', 'admin', 'hr_manager'), async (req, res) => {
  try {
    ...
    send500(res, 'Failed to toggle employee status', error);
  }
});
...
router.delete('/:id', requireRole('ceo', 'admin', 'hr_manager'), async (req, res) => {
  try {
    ...
    send500(res, 'Failed to deactivate employee', error);
  }
});
