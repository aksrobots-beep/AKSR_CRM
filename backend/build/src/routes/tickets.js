...
router.patch('/:id/assign', async (req, res) => {
  try {
    ...
    const currentTicket = await findById('tickets', req.params.id);
    if (!currentTicket) return res.status(404).json({ error: 'Ticket not found' });
    const updates = { assigned_to, updated_at: now, updated_by: req.user.id };
    ...
    await insert('audit_logs', {
      ...
      action: 'assignment',
      previous_value: currentTicket.assigned_to || 'unassigned',
      new_value: assigned_to || 'unassigned',
      user_id: req.user.id,
      timestamp: now,
    });
    const ticket = await update('tickets', req.params.id, updates);
...
router.patch('/:id/status', async (req, res) => {
  try {
    ...
    await insert('audit_logs', {
      ...
      previous_value: currentTicket.status,
      new_value: status,
      user_id: req.user.id,
      timestamp: now,
    });
...
router.patch('/:id/active', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to toggle ticket status' });
  }
});
...
router.get('/:id/parts', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to get ticket parts' });
  }
});
...
router.post('/:id/parts', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to add parts to ticket' });
  }
});
