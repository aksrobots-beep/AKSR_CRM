...
async function initMySQL() {
  ...
  try {
    ...
    if (existingUsers[0].count > 0) {
      ...
      // Users
      const users = [
        { id: uuidv4(), email: 'ceo@aksuccess.com', password: hashedPassword, name: 'Ahmad Khalid', role: 'ceo', department: 'Executive' },
        { id: uuidv4(), email: 'admin@aksuccess.com', password: hashedPassword, name: 'Sarah Tan', role: 'admin', department: 'Operations' },
        { id: uuidv4(), email: 'manager@aksuccess.com', password: hashedPassword, name: 'David Wong', role: 'service_manager', department: 'Service' },
        { id: uuidv4(), email: 'tech@aksuccess.com', password: hashedPassword, name: 'Rajan Kumar', role: 'technician', department: 'Service' },
        { id: uuidv4(), email: 'hr@aksuccess.com', password: hashedPassword, name: 'Lisa Chen', role: 'hr_manager', department: 'HR' },
        { id: uuidv4(), email: 'finance@aksuccess.com', password: hashedPassword, name: 'Michael Lee', role: 'finance', department: 'Finance' },
      ];

      for (const user of users) {
        await connection.execute(
          'INSERT INTO users (id, email, password, name, role, department, is_active, created_at) VALUES (?, ?, ?, ?, ?, ?, 1, ?)',
          [user.id, user.email, user.password, user.name, user.role, user.department, now]
        );
      }
